...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (signupUser) {
                const user = await axios.post('http://localhost:3000/api/users/create-user', formData);
                if (user.data.token) {
                ...
                const user = await axios.post('http://localhost:3000/api/users/verify-login', formData);
                if (user.data.token) {
                    login(user.data.token);
        ...
        <div className="relative flex justify-center items-center h-screen overflow-hidden bg-gradient-to-br from-blue-200 via-purple-200 to-pink-200">
            ...
            >
                ...
                <p className="text-center text-sm mt-2 text-gray-700">
                    ...
                    <span
                        onClick={() => setSignupUser(!signupUser)}
